...
namespace {
/* UI framebuffer capture, for automated inspection from JS.
 *
 * The canvas is an OffscreenCanvas owned by the render thread, so the page
 * cannot read a single pixel of it: no getImageData, no toDataURL, nothing.
 * That left visual bugs in the INTERFACE (as opposed to the 3D viewport, which
 * the render path already reads back) with no way to detect them except a human
 * looking at a screenshot. This copies the composited backbuffer into a plain
 * RGBA8 buffer that JS can read out of the wasm heap, which turns "a label is
 * sometimes missing after a resize" into something a loop can measure.
 *
 * Off unless requested; one frame per request. */
volatile int g_ui_capture_request = 0;
uint8_t *g_ui_pixels = nullptr;
uint32_t g_ui_w = 0, g_ui_h = 0;
volatile int g_ui_ready = 0; /* 0 = pending, 1 = ok, -1 = failed */
...
void WebGPUContext::render_pass_ensure(WebGPUFrameBuffer &fb)
{
  ...
  render_pass_end();
  if (debug_watch_tex_ != nullptr && debug_watch_budget_ > 0 && getenv("WGPU_DEPTH_WATCH")) {
    debug_watch_budget_--;
    std::string t = std::string("rp-begin:") + fb.name_get();
    debug_depth_stats_tex(debug_watch_tex_, t.c_str());
  }
...
/* WEB BUILD -- see blender/LOCAL_CHANGES.md. Depth statistics probe.
 * Copies the depth aspect of `fb`'s depth attachment into a MapRead buffer and
 * prints min/max/coverage from the async map callback, entirely on this
 * (render) thread -- no JS involvement, unlike the film capture path. Used to
 * tell "the depth was never written" from "the depth is compared wrongly". */
namespace {
struct DepthStatJob {
  WGPUBuffer buf;
  uint32_t w, h, bpr;
...
void depth_stat_map_cb(WGPUMapAsyncStatus status, WGPUStringView /*msg*/, void *u1, void * /*u2*/)
{
  DepthStatJob *job = static_cast<DepthStatJob *>(u1);
  if (job == nullptr) {
    return;
  }
  if (status == WGPUMapAsyncStatus_Success) {
    const size_t size = size_t(job->bpr) * job->h;
    const uint8_t *base = static_cast<const uint8_t *>(wgpuBufferGetConstMappedRange(job->buf, 0, size));
    if (base != nullptr) {
      float mn = 1e30f, mx = -1e30f;
      uint32_t lt1 = 0, nan = 0, total = 0;
      /* Coarse spatial picture: min depth per cell of a 6-row x 10-col grid
       * (row 0 = first rows in memory = GL bottom because of the y-flip). */
      float cell_min[6][10];
      for (int i = 0; i < 6; i++) for (int j = 0; j < 10; j++) cell_min[i][j] = 2.0f;
      for (uint32_t y = 0; y < job->h; y++) {
        const float *row = reinterpret_cast<const float *>(base + size_t(y) * job->bpr);
        const int ci = int((uint64_t(y) * 6) / job->h);
        for (uint32_t x = 0; x < job->w; x++) {
          const float v = row[x];
          total++;
          if (v != v) { nan++; continue; }
          if (v < mn) mn = v;
          if (v > mx) mx = v;
          if (v < 1.0f) lt1++;
          const int cj = int((uint64_t(x) * 10) / job->w);
          if (v < cell_min[ci][cj]) cell_min[ci][cj] = v;
        }
      }
      const float *crow = reinterpret_cast<const float *>(base + size_t(job->h / 2) * job->bpr);
      fprintf(stderr,
              "WGPU_DEPTHSTAT #%d sh='%s' fb='%s' %ux%u min=%.7f max=%.7f lt1(strict)=%u/%u nan=%u center=%.7f\n",
              job->seq, job->tag, job->fb, job->w, job->h, mn, mx, lt1, total, nan, crow[job->w / 2]);
      for (int i = 5; i >= 0; i--) {
        fprintf(stderr, "WGPU_DEPTHSTAT #%d row%d:", job->seq, i);
        for (int j = 0; j < 10; j++) fprintf(stderr, " %.5f", cell_min[i][j]);
        fprintf(stderr, "\n");
      }
      fflush(stderr);
    }
    wgpuBufferUnmap(job->buf);
  }
  else {
    fprintf(stderr, "WGPU_DEPTHSTAT sh='%s' fb='%s' map FAILED\n", job->tag, job->fb);
    fflush(stderr);
  }
  wgpuBufferRelease(job->buf);
  delete job;
}
}  // namespace

void WebGPUContext::debug_depth_stats(WebGPUFrameBuffer &fb, const char *tag)
{
  WebGPUTexture *dt = static_cast<WebGPUTexture *>(fb.depth_tex());
  if (dt == nullptr) {
    fprintf(stderr, "WGPU_DEPTHSTAT sh='%s' fb='%s' no depth attachment\n", tag, fb.name_get());
    fflush(stderr);
    return;
  }
  std::string t = std::string(tag) + " fb=" + fb.name_get();
  debug_depth_stats_tex(dt, t.c_str());
}

void WebGPUContext::debug_depth_stats_tex(WebGPUTexture *dt, const char *tag)
{
  if (device_ == nullptr || dt == nullptr || dt->wgpu_texture() == nullptr) {
    fprintf(stderr, "WGPU_DEPTHSTAT sh='%s' no texture\n", tag);
    fflush(stderr);
    return;
  }
  const WGPUTextureFormat fmt = dt->wgpu_format();
  if (!(fmt == WGPUTextureFormat_Depth32Float || fmt == WGPUTextureFormat_Depth32FloatStencil8)) {
    fprintf(stderr, "WGPU_DEPTHSTAT sh='%s' unsupported depth fmt=%d\n", tag, int(fmt));
    fflush(stderr);
    return;
  }
  DepthStatJob *job = new DepthStatJob();
  job->w = uint32_t(std::max(dt->width_get(), 1));
  job->h = uint32_t(std::max(dt->height_get(), 1));
  job->bpr = align256(job->w * 4u);
  snprintf(job->tag, sizeof(job->tag), "%s", tag ? tag : "?");
  snprintf(job->fb, sizeof(job->fb), "%s@%p", dt->debug_name().c_str(), (void *)dt->wgpu_texture());
  static int s_seq = 0;
  job->seq = ++s_seq;
  WGPUBufferDescriptor md = {};
  md.label = {"depth_stat_map", WGPU_STRLEN};
  md.size = uint64_t(job->bpr) * uint64_t(job->h);
  md.usage = WGPUBufferUsage_CopyDst | WGPUBufferUsage_MapRead;
  job->buf = wgpuDeviceCreateBuffer(device_, &md);
  if (job->buf == nullptr) {
    delete job;
    return;
  }
  ...
  WGPUTexelCopyTextureInfo src = {};
  src.texture = dt->wgpu_texture();
  src.aspect = WGPUTextureAspect_DepthOnly;
  WGPUTexelCopyBufferInfo dst = {};
  dst.buffer = job->buf;
  dst.layout.bytesPerRow = job->bpr;
  dst.layout.rowsPerImage = job->h;
  WGPUExtent3D ext = {job->w, job->h, 1};
  wgpuCommandEncoderCopyTextureToBuffer(enc, &src, &dst, &ext);
  /* ENV.WGPU_DEPTH_STATS_NOFLUSH=1 keeps the copy inside the batched encoder so
   * the probe observes the real submission order instead of perturbing it. */
  if (!getenv("WGPU_DEPTH_STATS_NOFLUSH")) {
    flush_encoder();
  }
  WGPUBufferMapCallbackInfo ci = {};
  ci.mode = WGPUCallbackMode_AllowSpontaneous;
  ci.callback = depth_stat_map_cb;
  ci.userdata1 = job;
  wgpuBufferMapAsync(job->buf, WGPUMapMode_Read, 0, size_t(job->bpr) * job->h, ci);
}

void WebGPUContext::ui_capture_if_requested()
{
  if (g_ui_capture_request == 0 || device_ == nullptr || backbuffer_color_ == nullptr) {
    return;
  }
  g_ui_capture_request = 0;
  g_ui_ready = 0;

  WGPUTexture tex = static_cast<WebGPUTexture *>(backbuffer_color_)->wgpu_texture();
  if (tex == nullptr) {
    g_ui_ready = -1;
    return;
  }
  const uint32_t w = uint32_t(backbuffer_w_);
  const uint32_t h = uint32_t(backbuffer_h_);
  const uint32_t bpp = 4; /* backbuffer is RGBA8Unorm, see present_backbuffer */
  const uint32_t bpr = align256(w * bpp);
  const uint64_t size = uint64_t(bpr) * uint64_t(h);

  WGPUBufferDescriptor md = {};
  md.label = {"ui_capture", WGPU_STRLEN};
  md.size = size;
  md.usage = WGPUBufferUsage_CopyDst | WGPUBufferUsage_MapRead;
  WGPUBuffer map_buf = wgpuDeviceCreateBuffer(device_, &md);
  if (map_buf == nullptr) {
  ...
  UiMapCtx *mc = new UiMapCtx{map_buf, w, h, bpr};
  WGPUBufferMapCallbackInfo cb = {};
  cb.mode = WGPUCallbackMode_AllowSpontaneous;
  cb.userdata1 = mc;
  cb.callback = [](WGPUMapAsyncStatus status, WGPUStringView, void *ud1, void *) {
    UiMapCtx *mc = static_cast<UiMapCtx *>(ud1);
    if (status == WGPUMapAsyncStatus_Success) {
      const uint8_t *raw = static_cast<const uint8_t *>(
          wgpuBufferGetConstMappedRange(mc->buf, 0, uint64_t(mc->bpr) * uint64_t(mc->h)));
      if (raw) {
        free(g_ui_pixels);
        g_ui_pixels = static_cast<uint8_t *>(malloc(size_t(mc->w) * mc->h * 4));
        if (g_ui_pixels) {
          /* Repack to a tight RGBA8 image: the copy above is padded to a
           * 256-byte row alignment, which JS should not have to know about. */
          for (uint32_t y = 0; y < mc->h; y++) {
            std::memcpy(g_ui_pixels + size_t(y) * mc->w * 4,
                        raw + size_t(y) * mc->bpr,
                        size_t(mc->w) * 4);
          }
          g_ui_w = mc->w;
...
void WebGPUContext::present_backbuffer(int w, int h)
{
  ...
  /* Before the surface blit: backbuffer_color_ holds the finished frame. */
  ui_capture_if_requested();
  if (surface_ == nullptr) {
...
bool WebGPUContext::map_read_sync(WGPUBuffer buf, size_t size, void *dst)
{
  ...
  if (ws != WGPUWaitStatus_Success || st.status != WGPUMapAsyncStatus_Success) {
    /* WEB BUILD: a blocking map needs JSPI or Asyncify, and this module is
     * built with neither (JSPI is incompatible with -fexceptions, which Blender
     * needs). The failure is therefore a property of the build, not of this
     * buffer: latch it so per-frame callers stop paying for a readback that
     * cannot succeed. */
    sync_map_possible_ = false;
...
extern "C" {
/* UI capture, called from the page's main thread (the globals are shared
 * memory, same as the MCP mailbox): request, poll ready, read w*h*4 bytes. */
EMSCRIPTEN_KEEPALIVE void wgpu_ui_capture_request() { g_ui_capture_request = 1; }
EMSCRIPTEN_KEEPALIVE int wgpu_ui_capture_ready() { return g_ui_ready; }
EMSCRIPTEN_KEEPALIVE int wgpu_ui_capture_w() { return int(g_ui_w); }
EMSCRIPTEN_KEEPALIVE int wgpu_ui_capture_h() { return int(g_ui_h); }
EMSCRIPTEN_KEEPALIVE uint8_t *wgpu_ui_capture_ptr() { return g_ui_pixels; }
