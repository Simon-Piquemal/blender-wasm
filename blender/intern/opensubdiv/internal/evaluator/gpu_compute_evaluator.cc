...
/* WEB BUILD -- see blender/LOCAL_CHANGES.md. Upstream includes <epoxy/gl.h>
 * here, a leftover from the days when this evaluator talked to GL directly: no
 * line of this file calls a GL entry point any more, everything goes through
 * Blender's GPU abstraction (GPU_compute / GPU_storage_buffer / shader create
 * infos). The include is unconditional while epoxy is only located when
 * WITH_OPENGL_BACKEND is on, so with the WebGPU backend it is the single thing
 * standing between WITH_OPENSUBDIV=ON and a successful compile. Dropped. */
