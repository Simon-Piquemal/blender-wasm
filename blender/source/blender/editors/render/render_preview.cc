...
/* Web diagnostic: the icon preview job runs SYNCHRONOUSLY under emscripten
 * (see wm_jobs.cc), so anything that blocks inside it freezes the whole tab.
 * BLENDER_WEB_LOG_PREVIEW=1 traces the stages to say which one never returns. */
#ifdef __EMSCRIPTEN__
static void preview_web_log(const char *msg)
{
  if (getenv("BLENDER_WEB_LOG_PREVIEW")) {
    fprintf(stderr, "PREVIEW_STAGE %s\n", msg);
    fflush(stderr);
  }
}
#else
static void preview_web_log(const char * /*msg*/) {}
#endif
#define PREVIEW_WEB_LOG(msg) preview_web_log(msg)
...
static void shader_preview_render(ShaderPreview *sp, ID *id, int split, int first)
{
  ...
  PREVIEW_WEB_LOG("prepare_scene:enter");
  sce = preview_prepare_scene(sp->bmain, sp->scene, id, idtype, sp);
  PREVIEW_WEB_LOG("prepare_scene:done");
  if (sce == nullptr) {
    return;
  }
  ...
  if (idtype == ID_TE) {
    ...
    PREVIEW_WEB_LOG("RE_PreviewRender:enter");
    RE_PreviewRender(re, pr_main, sce);
    PREVIEW_WEB_LOG("RE_PreviewRender:done");
  }
  ...
  if (sp->pr_method == PR_ICON_RENDER) {
    if (sp->pr_rect) {
      PREVIEW_WEB_LOG("ResultGet32:enter");
      RE_ResultGet32(re, reinterpret_cast<uint8_t *>(sp->pr_rect));
      PREVIEW_WEB_LOG("ResultGet32:done");
    }
  }
...
/* Preview renders are OFF in the browser build. Two reasons, both measured:
 *
 * 1. They freeze the tab. Under emscripten a WM job runs SYNCHRONOUSLY on the
 *    main thread (wm_jobs.cc -- worker pthreads cannot touch WebGPU), and one
 *    material icon drags in a full EEVEE pipeline. Tracing an actual "open the
 *    Material tab" with BLENDER_WEB_LOG_PREVIEW=1 counted 34 shaders compiled
 *    back to back for a 32x32 icon, 16.5 s of Tint translation alone, with the
 *    whole interface unresponsive throughout. On a colder WGSL cache it is far
 *    worse. Upstream already anticipates this exact failure -- see the linked-ID
 *    workaround just above, whose comment names Eevee shader compilation.
 *
 * 2. The pixels are discarded anyway. The render completes and the film reports
 *    "WGPU_FILM inject: no Render for scene", so RE_ResultGet32 finds nothing
 *    and the thumbnail never reaches the icon. The freeze buys a blank square.
 *
 * What is lost is thumbnails: the material/world/texture icon falls back to the
 * generic one. Material Preview and Rendered viewport shading are a different
 * path and are untouched, as are object, collection and image previews, which
 * are solid-mode or straight from an ImBuf and cost nothing.
 *
 * BLENDER_WEB_PREVIEW_RENDER=1 puts them back. Re-enabling for real means
 * seeding the preview scene's shaders into wgsl-cache.json (kills reason 1) and
 * fixing the film injection (kills reason 2). */
static bool preview_render_allowed_on_web()
{
#ifdef __EMSCRIPTEN__
  return getenv("BLENDER_WEB_PREVIEW_RENDER") != nullptr;
#else
  return true;
#endif
}
...
static void icon_preview_startjob_all_sizes(void *customdata, wmJobWorkerStatus *worker_status)
{
  ...
  for (int i = 0; i < NUM_ICON_SIZES; i++) {
    ...
    PREVIEW_WEB_LOG("other_id_types:enter");
    other_id_types_preview_render(prv, ip, icon_size, pr_method, worker_status);
    PREVIEW_WEB_LOG("other_id_types:done");
  }
}
                           ...
                           ePreviewRenderMethod method)
{
  ...
  if (preview_method_is_render(method) && !preview_render_allowed_on_web()) {
    return;
  }
