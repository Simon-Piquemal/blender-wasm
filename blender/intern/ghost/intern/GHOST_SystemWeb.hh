...
class GHOST_SystemWeb : public GHOST_System {
 ...
 public:
  ...
  {
    /* GPUReadFrontBuffer is claimed by GHOST_CAPABILITY_FLAG_ALL and is NOT
     * true here: there is no front buffer to read. The canvas is presented by
     * copying our own backbuffer into a surface texture, and nothing keeps the
     * presented image around afterwards.
     *
     * Claiming it made WM_window_pixels_read() take the front-buffer branch,
     * which aborted the wasm module with "memory access out of bounds" -- a
     * crash that took the whole tab down, with no Blender error and no stack.
     * It reached two features: Save Screenshot, and the colour picker, which
     * samples the window the same way through WM_window_pixels_read_sample().
     *
     * Dropping the flag routes both to the offscreen path, which redraws the
     * window into a texture it owns and reads that back instead. */
