...
void WebGPUStorageBuf::read(void *data)
{
  ...
  WebGPUContext *ctx = webgpu_context_get();
  /* WEB BUILD: skip the whole GPU round-trip once a blocking map has been shown
   * to be impossible (no JSPI/Asyncify in this module -- see map_read_sync).
   * This runs EVERY FRAME from the draw manager, and each attempt cost a guard
   * flush, a staging-buffer allocation, a copy, a second queue submit and a
   * WaitAny that returns immediately -- all of it discarded before falling back
   * to the host mirror below. The mirror is what the caller ends up with either
   * way, so the only thing lost is the wasted work. */
  if (ctx != nullptr && !ctx->sync_map_possible()) {
    if (data_) {
