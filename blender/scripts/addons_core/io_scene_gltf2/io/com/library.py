...
def dll_exists(lib_name, lib_display_name) -> bool:
    ...
    if exists:
        ...
        # dll_path() already said why: this platform has no shared-library
        # naming convention it knows (sys.platform == 'emscripten' here). The
        # branch below dereferenced the None it had just been handed, so glTF
        # export died with "AttributeError: 'NoneType' object has no attribute
        # 'absolute'" instead of simply reporting that Draco is unavailable --
        # which is the correct answer, since a WebAssembly build cannot load a
        # native library at all.
        print('INFO', '{} is not available on this platform'.format(lib_display_name))
    else:
        print(
            'ERROR',
