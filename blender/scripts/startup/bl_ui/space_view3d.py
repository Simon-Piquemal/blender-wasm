...
#
# Which parts of the 3D view header to draw. These blocks are inline in
# VIEW3D_HT_header.draw(), so unlike the top bar or the Properties tabs there is
# no class to unregister and no RNA flag to clear: guarding them here is the
# only way that does not mean copying the whole 280-line draw() into
# scripts/startup/webapp_ui.py and letting it rot out of sync.
WEBAPP_HEADER = {
    "editor_menus": False,   # View / Select / Add / Object
    "xform": False,          # transform orientation, snapping, proportional editing
    "object_visibility": False,  # Selectability & Visibility popover
    "gizmos": False,         # Show Gizmo toggle + popover
    "overlays": False,       # Show Overlays toggle + popover
    "xray": False,           # Toggle X-Ray
    "shading": True,         # shading mode buttons (kept)
}
...
class VIEW3D_HT_header(Header):
    ...
    def draw(self, context):
        ...
        if object_mode in {'PAINT_GREASE_PENCIL', 'SCULPT_GREASE_PENCIL'}:
            ...
            if WEBAPP_HEADER["xform"]:
                VIEW3D_HT_header.draw_xform_template(layout, context)

        layout.separator_spacer()

        # Viewport Settings
        if WEBAPP_HEADER["object_visibility"]:
            layout.popover(
