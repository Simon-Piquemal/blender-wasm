...
{
#ifdef GPU_WEBGPU
  /* WEB BUILD: write the film depth as-is, without the conservative expansion below.
   *
   * Chain being matched here: `film.depth_img` holds linear scene depth, it is converted back
   * with `ViewMatrices::depth_view_to_screen()` which already ends on `ndc_to_screen()`, i.e.
   * `z_ndc * 0.5f + 0.5f`, so the value handed to `gl_FragDepth` is ALREADY in the [0..1]
   * range WebGPU expects. The backend's vertex wrapper (`gl_Position.z = (z + w) * 0.5f`, see
   * `webgpu_shader.cc`) performs the exact same remap for rasterized geometry and is bypassed
   * for a shader-written `gl_FragDepth`, so both paths agree: there is NO missing convention
   * conversion here, and this amend is the only systematic difference left between the depth
   * EEVEE's film writes and the depth the rasterizer would have produced for the same surface.
   *
   * Upstream adds it so that the depth of the *jittered* render sample can never end up nearer
   * than the true pixel-center depth (TAA jitters the projection by up to +/-0.5 px, so the
   * sampled depth can be off by up to 0.5 * fwidth in either direction).
   *
   * Magnitude on our scale: fwidth(z_screen) ~= (1 - z_screen) * pixel_angular_size * slope.
   * For a surface measured at z_screen = 0.99942 (near = 0.01, far = 1000, ~17 BU away) that
   * is ~6e-4 * 1e-3 * slope, i.e. ~5e-7 on a 45 degree facing face; the constant term adds
   * 4 * 2.4e-7 = 9.6e-7. Total ~1.5e-6 out of the 5.8e-4 that separates that surface from the
   * far plane. Dropping it cannot hide surface overlays: every overlay drawn on top of solid
   * geometry carries its own, far larger, bias (outline prepass: -1e-3 in clip space, i.e.
   * -2.9e-5 here; floor grid: +2.5e-4 .. -2.0e-4 in clip space).
   *
   * Removing it also removes the halo it creates at silhouettes, where the 2x2 quad straddles
   * geometry and background and fwidth jumps to (1 - z_screen). */
                  ...
                  [[frag_depth(any)]] float out_depth)
{
  ...
  out_depth = views.get(0).depth_view_to_screen(-out_depth);
  /* WEB BUILD: was an inlined copy of `display_depth_amend()`. Call the shared helper instead so
   * both film entry points (`accumulate_or_display_frag` and this one, used by
   * `eevee_film_copy_frag`) stay in sync, in particular for the GPU_WEBGPU path. */
  out_depth = display_depth_amend(out_depth);
}
