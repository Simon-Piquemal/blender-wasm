...
/* A counter rather than a log line, because the page console drops lines under
 * load and a dropped line reads exactly like "the interface never asked to draw
 * this", which is the very thing being measured. JS reads this directly. */
volatile int g_web_uitext_hits = 0;
volatile int g_web_uitext_empty = 0;
/* Clipping outcomes for the label being followed, as counters for the same
 * reason as above: the console drops lines, a counter cannot lie by omission. */
volatile int g_web_clip_lost = 0;
volatile int g_web_clip_shrunk = 0;
volatile int g_web_clip_okwidth = 0;

extern "C" {
EMSCRIPTEN_KEEPALIVE int blender_web_uitext_count() { return g_web_uitext_hits; }
EMSCRIPTEN_KEEPALIVE int blender_web_uitext_empty() { return g_web_uitext_empty; }
EMSCRIPTEN_KEEPALIVE void blender_web_uitext_reset() { g_web_uitext_hits = 0; g_web_uitext_empty = 0; }
EMSCRIPTEN_KEEPALIVE int blender_web_clip_lost() { return g_web_clip_lost; }
EMSCRIPTEN_KEEPALIVE int blender_web_clip_shrunk() { return g_web_clip_shrunk; }
EMSCRIPTEN_KEEPALIVE int blender_web_clip_okwidth() { return g_web_clip_okwidth; }
}
...
static void text_clip_middle(const uiFontStyle *fstyle, Button *but, const rcti *rect)
{
  ...
  /* WEB BUILD debug: ENV.BLENDER_WEB_LOG_CLIP=1 reports a button label that goes
   * in non-empty and comes out empty (or as just the ellipsis). Chasing "the
   * Add Modifier label vanishes after a window resize until you hover it": this
   * says whether the text is lost here, on the CPU, before it ever reaches the
   * GPU -- BLF_width() misreporting, or a stale button rect making okwidth too
   * small. */
#ifdef __EMSCRIPTEN__
  /* Same substring filter as the widget-text probe, counted rather than logged.
   * This is the last thing between "the interface asked to draw this label" and
   * "the glyphs are queued": if the label arrives here intact and leaves empty
   * or shortened, the text was lost on the CPU and the GPU path is innocent. */
  {
    static const char *clip_filter = getenv("BLENDER_WEB_LOG_TEXT");
    if (clip_filter != nullptr && !but->drawstr.empty() &&
        (clip_filter[0] == 0 || strcmp(clip_filter, "1") == 0 ||
         strstr(but->drawstr.c_str(), clip_filter) != nullptr))
    {
      const size_t before = but->drawstr.size();
      const size_t after = strlen(new_drawstr);
      g_web_clip_okwidth = int(okwidth);
      if (after == 0 || STREQ(new_drawstr, BLI_STR_UTF8_HORIZONTAL_ELLIPSIS)) {
        g_web_clip_lost++;
      }
  ...
  if (getenv("BLENDER_WEB_LOG_CLIP")) {
    const bool was_empty = but->drawstr.empty();
    const bool now_empty = (new_drawstr[0] == '\0') ||
                           STREQ(new_drawstr, BLI_STR_UTF8_HORIZONTAL_ELLIPSIS);
    if (!was_empty && now_empty) {
      fprintf(stderr,
              "WEB_CLIP lost '%s' okwidth=%.1f minwidth=%.1f rect=%dx%d strwidth=%.1f\n",
              but->drawstr.c_str(),
              okwidth,
              minwidth,
...
/* Web diagnostic: does the interface even ASK to draw this label?
 *
 * The resize text-loss bug was narrowed to "the region's own offscreen does not
 * contain the pixels", which puts the question here rather than in the GPU
 * backend: on the frame that loses a label, is widget_draw_text called for it
 * at all, and with what rectangle? Frames can be told apart by the
 * WGPU_STATS frame=N line the backend prints once per present.
 * ENV.BLENDER_WEB_LOG_TEXT=1. */
#ifdef __EMSCRIPTEN__

static void web_log_widget_text(const char *str, const rcti *rect)
{
  /* The value is a substring filter, not just a switch: logging every widget's
   * text drowns the page console, which drops lines, which is worse than no
   * measurement at all. BLENDER_WEB_LOG_TEXT=Add Modifier follows one label. */
  static const char *filter = getenv("BLENDER_WEB_LOG_TEXT");
  if (filter == nullptr || str == nullptr || str[0] == 0) {
    return;
  }
  if (filter[0] != 0 && strcmp(filter, "1") != 0 && strstr(str, filter) == nullptr) {
    return;
  }
  g_web_uitext_hits++;
  ...
  fprintf(stderr,
          "UI_TEXT '%.40s' rect=%d,%d,%d,%d w=%d h=%d%c",
          str,
...
#else
static void web_log_widget_text(const char *, const rcti *) {}
#endif
                             ...
                             rcti *rect)
{
  ...
  web_log_widget_text(drawstr, rect);
