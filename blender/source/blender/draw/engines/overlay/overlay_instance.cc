...
void Instance::init()
{
  ...
  if (state.v3d != nullptr) {
    ...
    /* WEB BUILD -- see blender/LOCAL_CHANGES.md. On the WebGPU backend EEVEE's
     * depth never reaches the overlay's depth attachment, so trusting it means
     * overlays test against an EMPTY buffer and draw over the geometry -- the
     * "grid ghosts through the cube in Material Preview" bug. Measured with the
     * backend's depth probe (WGPU_DEPTH_STATS=overlay_grid_next), reading the
     * exact buffer the grid tests against, at the same pixel, same moment:
     *
     *   Solid (workbench) : min=0.99944  center=0.99947  18068 texels < 1
     *   Material (EEVEE)  : min=1.0      center=1.0          0 texels < 1
     *
     * Nothing else explains it: the ghost is gone if the grid is hidden, it
     * reproduces in orthographic (where the grid's multi-iteration depth bias
     * is not applied at all), the bind-group cache keys on the push-constant
     * slice offset so `grid_iter` is versioned per draw, and forcing
     * one-submit-per-dispatch changes nothing.
     *
     * Declaring the depth unavailable takes the fallback upstream already has
     * for engines whose depth cannot be trusted: the overlay clears the buffer
     * and fills it with its own depth prepass (see the `dt >= OB_SOLID` branch
     * of object_needs_prepass()). It costs one prepass of the scene geometry
     * per frame -- which is exactly what workbench-grade occlusion needs.
     * ENV.BLENDER_WEB_EEVEE_DEPTH=1 restores the upstream assumption. */
    if (viewport_uses_eevee && !viewport_uses_workbench &&
        getenv("BLENDER_WEB_EEVEE_DEPTH") == nullptr)
    {
      state.is_render_depth_available = false;
    }
