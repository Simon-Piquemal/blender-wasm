...
ImageSpec imb_create_write_spec(const WriteContext &ctx, int file_channels, TypeDesc data_format)
{
  ...
  /* PNG on wasm: attaching ANY resolution metadata makes libpng abort the file
   * with "tEXt: invalid keyword", so no IDAT is ever written and every PNG save
   * fails -- renders, screenshots, image export, the lot.
   *
   * OIIO's PNG writer is meant to consume these three attributes itself:
   * put_parameter() in png_pvt.h drops ResolutionUnit/XResolution/YResolution
   * before the generic "any leftover string becomes a tEXt chunk" branch. On
   * native builds it does. Here it does not, and the tEXt that results carries
   * an empty keyword, which libpng refuses.
   *
   * Established at runtime, not guessed: with IMB_DUMP_ATTRIBS=1 the spec for a
   * failing write holds exactly five attributes, of which ResolutionUnit is the
   * only string; a blank 8x8 image fails to save at the default 72 DPI and
   * saves in 189 bytes after `image.resolution = (0, 0)`.
   *
   * Which half of OIIO misbehaves -- the case-insensitive name compare or the
   * ustring backing the keyword -- is NOT isolated, so this does not pretend to
   * fix OIIO. It drops the pHYs chunk for PNG on this platform, which costs the
   * DPI hint and nothing else, and keeps PNG writable. */
#ifdef __EMSCRIPTEN__
  const bool skip_resolution_metadata = STREQ(ctx.file_format, "png");
#else
  const bool skip_resolution_metadata = false;
#endif

  if (!skip_resolution_metadata && ctx.ibuf->ppm[0] > 0.0 && ctx.ibuf->ppm[1] > 0.0) {
