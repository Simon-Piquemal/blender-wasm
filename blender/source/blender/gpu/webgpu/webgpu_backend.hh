...
class WebGPUBackend : public GPUBackend {
 ...
 public:
  ...
  void compute_dispatch(int groups_x_len, int groups_y_len, int groups_z_len) override
  {
    ...
    wgpuBindGroupLayoutRelease(bgl);
    /* WEB BUILD -- see blender/LOCAL_CHANGES.md. This used to submit here
     * unconditionally, one command buffer per dispatch, which is what made the
     * frame cost ~40 queue submits: EEVEE fires a dozen-plus compute passes per
     * frame (resource finalize, visibility, command generate, HiZ, light
     * culling, shadow tilemaps, light probes) and each one paid a JS/browser
     * round-trip. Measured: removing the shadow pool and the scene light cut
     * passes 41->36 and bind groups 132->90 while the frame time did not move
     * at all (30.3 ms both ways) -- the submits were the wall, not the work.
     *
     * Batching is safe here: within ONE command buffer WebGPU executes passes
     * in submission order and inserts the barriers between them itself, so a
     * compute pass writing a buffer that a later render pass reads is already
     * ordered. The write-ordering hazard this backend does have to guard --
     * wgpuQueueWrite* jumping ahead of recorded-but-unsubmitted work -- is
     * handled separately by flush_if_pass_open().
     *
     * ENV.WGPU_ISOLATE_COMPUTE=1 restores one-submit-per-dispatch, which is
     * worth having: a validation error poisons its whole command buffer and
     * takes every sibling pass down with it, so isolating dispatches is how you
     * find which one is at fault. */
    if (getenv("WGPU_ISOLATE_COMPUTE")) {
      ctx->flush_encoder();
    }
