...
/* WEB BUILD debug helper: does `env` (comma-separated substrings) match
 * `name`, with a per-token budget of 6 hits. */
static bool webgpu_depth_probe_match(const char *env, const char *name)
{
  const char *list = getenv(env);
  if (list == nullptr || name == nullptr) {
    return false;
  }
  static std::map<std::string, int> s_budget;
  int tok = 0;
  for (const char *p = list; *p; tok++) {
    const char *q = strchr(p, ',');
    const size_t n = q ? size_t(q - p) : strlen(p);
    if (n > 0 && strstr(name, std::string(p, n).c_str()) != nullptr) {
      int &b = s_budget[std::string(env) + "#" + std::string(p, n)];
      if (b < 6) {
        b++;
        return true;
      }
...
/* Web diagnostic: where does a TEXT draw go when its pixels never arrive?
 *
 * The resize text-loss bug was bracketed to "the draw is issued and the pixels
 * do not land" (see blender/LOCAL_CHANGES.md). record_draw() has three places
 * where it gives up silently, and one of them -- no render pass -- is exactly
 * what a mid-frame guard flush could produce, since uploading new glyph
 * bitmaps to the atlas ends the pass right before the text draw that samples
 * it. These count what actually happens, per shader, because a counter cannot
 * lie by omission the way a dropped log line can. */
volatile int g_web_text_draw_issued = 0;
volatile int g_web_text_draw_recorded = 0;
volatile int g_web_text_drop_pipeline = 0;
...
static bool web_is_text_shader(const WebGPUShader *sh)
{
  if (sh == nullptr) {
    return false;
  }
  const std::string &n = const_cast<WebGPUShader *>(sh)->name_get();
                              ...
                              intptr_t indirect_stride)
{
  ...
  const bool web_text = web_is_text_shader(shader);
  if (web_text) {
    g_web_text_draw_issued++;
  }
  ...
  /* WEB BUILD debug: ENV.WGPU_DEPTH_STATS=<substr,...> dumps the target's depth
   * BEFORE this draw; ENV.WGPU_DEPTH_STATS_AFTER=<substr,...> right AFTER it
   * (each token fires at most 6 times). */
  const bool probe_after = webgpu_depth_probe_match("WGPU_DEPTH_STATS_AFTER", shader->name_get().c_str());
  if (fb != nullptr && webgpu_depth_probe_match("WGPU_DEPTH_STATS", shader->name_get().c_str())) {
    ctx->debug_depth_stats(*fb, shader->name_get().c_str());
  }
  ...
  if (indirect_buf != nullptr) {
    ...
    if (v_count == 0 && web_text) {
      g_web_text_drop_novertex++;
    }
    if (v_count > 0) {
      if (web_text) {
        g_web_text_draw_recorded++;
      }
      if (expand_points) {
  ...
  /* WEB BUILD debug: ENV.WGPU_FLUSH_AFTER=<substr,...> submits right after the
   * matching draw (no readback) -- isolates batching/ordering bugs. */
  if (const char *fa = getenv("WGPU_FLUSH_AFTER")) {
    bool m = false;
    const char *nm = shader->name_get().c_str();
    for (const char *p = fa; *p && !m;) {
      const char *q = strchr(p, ',');
      const size_t n = q ? size_t(q - p) : strlen(p);
      m = n > 0 && strstr(nm, std::string(p, n).c_str()) != nullptr;
      p = q ? q + 1 : p + n;
    }
    if (m) {
      ctx->render_pass_end();
  ...
  if (probe_after && fb != nullptr) {
    std::string tag = std::string("after:") + shader->name_get().c_str();
    ctx->debug_depth_stats(*fb, tag.c_str());
    ctx->debug_watch_tex_ = static_cast<WebGPUTexture *>(fb->depth_tex());
  }
