...
/* numpy, linked statically.
 *
 * The wasm build has no dynamic loader, so a normal numpy install (a directory
 * of .so extension modules) cannot work: every one of them would need dlopen.
 * scripts/build_numpy.sh cross-compiles numpy and scripts/package_numpy.sh
 * archives the resulting objects into libnumpy.a and generates the table below,
 * which registers each extension under its full dotted name in CPython's
 * inittab -- the mechanism a static interpreter uses instead of a loader.
 *
 * Guarded by __has_include so a tree without numpy packaged still builds; the
 * feature is then simply absent rather than a broken link. The matching Python
 * side (a meta path finder, because BuiltinImporter refuses dotted submodules)
 * is scripts/startup/webapp_numpy.py.
 *
 * Without this, glTF and FBX -- both Python add-ons that import numpy -- fail on
 * their first line, which took the most useful export format on the web with
 * them. */
#if defined(__EMSCRIPTEN__) && defined(__has_include)
#  if __has_include("numpy_static_inittab.inc")
#    define BPY_NUMPY_STATIC 1
#    include "numpy_static_inittab.inc"
#  endif
#endif
...
#ifdef BPY_NUMPY_STATIC
  PyImport_ExtendInittab(numpy_inittab);
#endif
