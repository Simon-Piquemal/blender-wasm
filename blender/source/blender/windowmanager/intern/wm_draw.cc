...
static void wm_draw_region_unbind(ARegion *region)
{
  ...
  /* NOTE: flushing the pending text batch here (BLF_batch_draw_flush) was tried
   * against the resize text-loss bug and measured as no improvement -- 5
   * failures in 5 resize storms with it, 3 in 5 without. Reverted rather than
   * left in. See blender/LOCAL_CHANGES.md for what the measurement showed. */
...
#ifdef __EMSCRIPTEN__
/* Reading the window back is not possible in the browser build, and it is worth
 * being exact about why: there is no synchronous GPU readback here at all. A
 * texture is read by copying it into a buffer and mapping that buffer, and the
 * map completes on the browser event loop -- which cannot run while wasm is on
 * the stack, because this build has no JSPI (it is incompatible with
 * -fexceptions, see scripts/link_blender_web.sh). The render pipeline works
 * around it by deferring: it submits the copy and collects the pixels a frame
 * later. An operator that needs the pixels NOW has no such option.
 *
 * Two features land here, and neither can be honest about it on its own:
 * Save Screenshot and the colour picker, both through WM_window_pixels_read().
 * Left alone, the caller receives a buffer nobody filled and carries on: the
 * screenshot wrote a 1180x760 PNG of pure black, and the picker would sample
 * that same black. Returning nullptr makes the operator report a failure
 * instead, which is the truthful answer.
 *
 * Before this, the situation was worse than a black image: GHOST_SystemWeb
 * claimed GHOST_kCapabilityGPUReadFrontBuffer, so the read took the FRONT
 * buffer branch and aborted the wasm module outright -- "memory access out of
 * bounds", the whole tab gone. That claim is dropped in GHOST_SystemWeb.hh.
 *
 * For a picture of the interface, the backbuffer capture in
 * gpu/webgpu/webgpu_context.cc does work: it is asynchronous, so JS asks for a
 * frame and reads it back once the browser is free. */
static bool web_window_readback_supported()
{
  static int s_warned = 0;
  if (s_warned++ == 0) {
    fprintf(stderr,
            "WM_window_pixels_read: unsupported in the web build "
            "(no synchronous GPU readback without JSPI)\n");
    fflush(stderr);
  }
  return false;
}
...
#ifdef __EMSCRIPTEN__
  if (!web_window_readback_supported()) {
    r_size[0] = 0;
    r_size[1] = 0;
    return nullptr;
  }
