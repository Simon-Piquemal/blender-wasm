...
class WebGPUContext : public Context {
 ...
 public:
  ...
  /* WEB BUILD debug: copy a framebuffer's depth aspect out and print min/max
   * statistics from the map callback (ENV.WGPU_DEPTH_STATS=<shader substr>). */
  void debug_depth_stats(WebGPUFrameBuffer &fb, const char *tag);
  void debug_depth_stats_tex(WebGPUTexture *dt, const char *tag);
  /* WEB BUILD debug: texture captured by WGPU_DEPTH_STATS_AFTER, re-probed at
   * every render pass begin while ENV.WGPU_DEPTH_WATCH is set. */
  WebGPUTexture *debug_watch_tex_ = nullptr;
  int debug_watch_budget_ = 60;
  ...
  /* False once a blocking map has been proven impossible on this build, so
   * callers can skip the copy+submit they would only throw away. */
  bool sync_map_possible() const
  {
    return sync_map_possible_;
  }
  bool sync_map_possible_ = true;
 ...
 public:
  ...
  /* Copy the composited backbuffer into a JS-readable RGBA8 buffer when
   * wgpu_ui_capture_request() asked for it. One frame per request. */
  void ui_capture_if_requested();
