...
static gpu::Texture *blf_batch_cache_texture_load()
{
  ...
  if (gc->bitmap_len > gc->bitmap_len_landed) {
    ...
    /* NOTE: submitting here (GPU_flush) to pin the atlas upload ahead of the
     * draw that samples it was tried against the resize text-loss bug and
     * measured as NO improvement -- 3 failures in 5 resize storms either way.
     * Reverted rather than left in: it costs a submit on every frame that
     * rasterises a new glyph and buys nothing. See blender/LOCAL_CHANGES.md. */
  }

  return gc->texture;
}

/* Web diagnostic for "a label is missing after a resize, and hovering the
 * button brings it back". A resize is the only FULL interface redraw, so it is
 * the only moment with many text batches in flight, which is why it is the
 * only moment the bug appears. This prints one line per batch: how many glyphs
 * it carried, and whether it had an atlas texture to sample. A batch with
 * glyphs and no texture, or a batch silently dropped, is the thing to look for.
 * ENV.BLENDER_WEB_LOG_BLF=1. */
#ifdef __EMSCRIPTEN__
static void blf_web_log_batch(int glyph_len, const void *texture)
{
  static const bool on = getenv("BLENDER_WEB_LOG_BLF") != nullptr;
  if (!on) {
    return;
  }
  static int seq = 0;
  fprintf(stderr, "BLF_BATCH #%d glyphs=%d tex=%p%s\n",
          ++seq, glyph_len, texture, texture == nullptr ? "  <-- NO ATLAS" : "");
  fflush(stderr);
}
#else
static void blf_web_log_batch(int, const void *) {}
#endif
...
void blf_batch_draw()
{
  ...
  blf_web_log_batch(g_batch.glyph_len, texture);
  if (texture == nullptr) {
    /* Nothing to sample from: the draw would consume the queued glyphs and
     * paint nothing, and they are dropped on the floor below. Reset and let
     * the next redraw try again rather than losing them silently. */
