    ...
    case GHOST_kEventWindowMove: {
      ...
      if (state != GHOST_kWindowStateMinimized) {
        ...
        if (wm_window_update_size_position(win)) {
           ...
           *
           * NOTE: the browser was added here and measured as NO improvement to
           * the resize text-loss bug (5 failures in 6 storms with it, 3 in 5
           * without), so it was taken back out rather than left costing a full
           * draw per resize event. See blender/LOCAL_CHANGES.md. */
