...
bool GHOST_SystemWeb::handleResize()
{
   ...
   *
   * Resize the canvas BACKING STORE to match, not just Blender's notion of the
   * window. Nothing else can: the page transferred the canvas with
   * transferControlToOffscreen(), so assigning canvas.width from the main
   * thread throws InvalidStateError -- this render thread owns it now.
   *
   * Without this the drawing surface stayed frozen at its startup resolution
   * while CSS stretched it over the new window, and Blender re-laid its UI out
   * for a size the framebuffer did not have: every widget ended up somewhere
   * other than where it was painted, so clicks landed on the wrong thing. */
  emscripten_set_canvas_element_size(WEB_CANVAS, int(canvas_w_), int(canvas_h_));
