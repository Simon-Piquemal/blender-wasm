 ...
 *
 * Restrict which object modes the mode dropdown offers. An ALLOW-list, not a
 * deny-list: new modes appearing in a fork bump then stay hidden by default
 * instead of silently showing up.
 *
 * Filtered HERE, in the operator's itemf, and never by editing
 * rna_enum_object_mode_items: the 3D view header resolves the CURRENT mode with
 * enum_items[object_mode], so dropping an entry from that array would break the
 * header for any object already in that mode, and for any .blend saved in one.
 *
 * Comma-separated identifiers, default "OBJECT,EDIT". No rebuild needed to
 * change it: BLENDER_WEB_MODES="OBJECT,EDIT,POSE" (the demo forwards
 * window.__CAPENV into the environment); set it empty to allow every mode.
 * Full set: OBJECT EDIT POSE SCULPT VERTEX_PAINT WEIGHT_PAINT TEXTURE_PAINT
...
static bool object_mode_is_allowed(const char *identifier)
{
  const char *list = getenv("BLENDER_WEB_MODES");
  if (list == nullptr) {
    list = "OBJECT,EDIT";
  }
  if (list[0] == 0) {
    return true; /* explicitly empty: no filtering */
  }
  const size_t id_len = strlen(identifier);
  for (const char *p = list; *p != 0;) {
    const char *end = strchr(p, ',');
    const size_t len = (end != nullptr) ? size_t(end - p) : strlen(p);
    /* Length-checked, so "EDIT" never matches "EDIT_GPENCIL". */
  ...
  if (ob) {
    while (input->identifier) {
      if (mode_compat_test(ob, eObjectMode(input->value)) &&
          object_mode_is_allowed(input->identifier))
      {
